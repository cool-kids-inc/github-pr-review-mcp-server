...
class TestCommentFetching:
    ...
    async def test_fetch_pr_comments_safety_limits_pages(
        ...
        for page in range(5):  # Create 5 pages
            page_comments = [{"id": page * 10 + i, "body": f"Page {page} comment {i}"} for i in range(3)]
            headers = {}
...
class TestParameterValidation:
    ...
    async def test_environment_clamping_through_fetch(
        ...
        """Test that environment variables are properly clamped via fetch_pr_comments."""
...
class TestFileCreationSecurity:
    ...
    async def test_create_review_spec_file_filename_validation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("mcp_server.Path.cwd", return_value=Path(temp_dir)):
                ...
                result = await generator.create_review_spec_file([], "valid-name_123.md")
    ...
    async def test_create_review_spec_file_directory_escaping(self) -> None:
        ...
        result = await generator.create_review_spec_file([], "valid.md")
        # This mainly tests the relative_to() check in the implementation
        # Hard to test without actually creating symlinks, but the validation should catch it
...
class TestGitHubApiIntegration:
    ...
    def test_github_link_header_parsing(self) -> None:
        ...
        link_header = '<https://api.github.com/repos/o/r/pulls/123/comments?page=2>; rel="next", <https://api.github.com/repos/o/r/pulls/123/comments?page=5>; rel="last"'
        ...
        link_header_no_next = '<https://api.github.com/repos/o/r/pulls/123/comments?page=1>; rel="prev"'
...
class TestIntegrationScenarios:
    ...
    @pytest.mark.asyncio
    async def test_large_pr_with_many_comments(self, mock_http_client, github_token: str) -> None:
        ...
        for page in range(5):
            ...
            for i in range(20):  # 20 comments per page
                ...
                page_comments.append({
                    "id": comment_id,
                    "body": f"This is comment {comment_id} with some substantial content",
            ...
            if page < 4:
                mock_response.headers = {
                    "Link": f'<https://api.github.com/repos/owner/repo/pulls/123/comments?page={page+2}>; rel="next"'
                }
            else:
                mock_response.headers = {}
        ...
        # Fetch all comments
        comments = await fetch_pr_comments("owner", "repo", 123, max_pages=10, max_comments=200)
    ...
    @pytest.mark.asyncio
    async def test_network_failure_recovery_scenarios(self, mock_http_client, github_token: str) -> None:
