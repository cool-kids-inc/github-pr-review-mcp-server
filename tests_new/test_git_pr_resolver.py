...
class TestRemoteUrlParsing:
    ...
    def test_parse_https_urls(self) -> None:
        ...
        test_cases = [
            ...
            ("https://github.mycorp.com/team/project", ("github.mycorp.com", "team", "project")),
            ("https://ghe.example.com/user/repo.git", ("ghe.example.com", "user", "repo")),
        ]
    ...
    def test_parse_ssh_urls(self) -> None:
        ...
        test_cases = [
            ...
            ("git@github.mycorp.com:team/project.git", ("github.mycorp.com", "team", "project")),
...
class TestApiUrlConstruction:
    ...
    def test_api_base_with_environment_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestGraphqlUrlConstruction:
    ...
    def test_graphql_url_enterprise_patterns(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_graphql_url_explicit_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestGitRepositoryDetection:
    ...
    def test_git_repo_detection_from_remote(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("git_pr_resolver._get_repo") as mock_get_repo:
                ...
                mock_repo = Mock()
                mock_config = Mock()
                mock_config.get.return_value = b"https://github.com/test-owner/test-repo.git"
    ...
    def test_git_detection_detached_head_handling(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("git_pr_resolver._get_repo") as mock_get_repo:
                ...
                mock_repo.get_config.return_value = mock_config
                mock_repo.refs.read_ref.return_value = b"abc123"  # Detached HEAD (commit hash)
...
class TestPrUrlResolution:
    ...
    async def test_resolve_pr_branch_strategy_success(self, mock_http_client) -> None:
        ...
        mock_response = create_mock_response([
            {"number": 123, "head": {"ref": "feature-branch"}, "html_url": "https://github.com/owner/repo/pull/123"}
        ])
    ...
    async def test_resolve_pr_first_strategy(self, mock_http_client) -> None:
        ...
        mock_response = create_mock_response([
            ...
            {"number": 123, "html_url": "https://github.com/owner/repo/pull/123"},  # Should be selected
    ...
    async def test_resolve_pr_latest_strategy(self, mock_http_client) -> None:
        ...
        mock_response = create_mock_response([
            {"number": 456, "html_url": "https://github.com/owner/repo/pull/456", "updated_at": "2023-01-02T00:00:00Z"},
            {"number": 123, "html_url": "https://github.com/owner/repo/pull/123", "updated_at": "2023-01-01T00:00:00Z"},
        ])
        mock_http_client.add_get_response(mock_response)
    ...
    async def test_resolve_pr_graphql_fallback(self, mock_http_client) -> None:
        ...
        # Should have made both REST and GraphQL calls
        assert len(mock_http_client.get_calls) == 1  # REST
        assert len(mock_http_client.post_calls) == 1  # GraphQL
    ...
    async def test_resolve_pr_branch_not_found(self, mock_http_client) -> None:
        ...
        with pytest.raises(ValueError, match="No open PR found for branch"):
            await git_pr_resolver.resolve_pr_url("owner", "repo", "missing-branch", select_strategy="branch")
    ...
    @pytest.mark.asyncio
    async def test_resolve_pr_debug_logging(self, mock_http_client, debug_logging_enabled) -> None:
        ...
        # Should succeed despite GraphQL failure
        result = await git_pr_resolver.resolve_pr_url("owner", "repo")
        assert result == "https://github.com/owner/repo/pull/123"
        ...
        mock_response = create_mock_response({
            "data": {
                "repository": {
                    "pullRequests": {
                        "nodes": [
                            {"number": 456, "headRefName": "feature-branch"}
                        ]
                    }
                }
            }
