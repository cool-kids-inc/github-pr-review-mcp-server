...
@pytest.mark.asyncio
async def test_resolve_pr_url_branch_graphql_success(monkeypatch: Any) -> None:
    class DummyResp:
        def __init__(self, json_data: Union[Dict[str, Any], List[Dict[str, Any]]], status_code: int = 200) -> None:
    ...
    class FakeClient:
        ...
        async def post(self, url: str, json: Optional[Dict[str, Any]] = None, headers: Optional[Dict[str, str]] = None) -> DummyResp:  # noqa: A002
        ...
        async def get(self, url: str, headers: Optional[Dict[str, str]] = None) -> DummyResp:
...
@pytest.mark.asyncio
async def test_resolve_pr_url_branch_graphql_errors_fallback_rest(monkeypatch: Any) -> None:
    class DummyResp:
        def __init__(self, json_data: Union[Dict[str, Any], List[Dict[str, Any]]], status_code: int = 200) -> None:
            self._json = json_data
            self.status_code = status_code
        ...
        def raise_for_status(self) -> None:
            if self.status_code >= 400:
                request = Mock(spec=httpx.Request); response = Mock(spec=httpx.Response); raise httpx.HTTPStatusError("error", request=request, response=response)
...
@pytest.mark.asyncio
async def test_graphql_url_inference_on_ghe(monkeypatch: Any) -> None:
    class DummyResp:
        def __init__(self, json_data: Union[Dict[str, Any], List[Dict[str, Any]]], status_code: int = 200) -> None:
            self._json = json_data
            self.status_code = status_code
