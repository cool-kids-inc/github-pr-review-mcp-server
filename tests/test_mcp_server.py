...
class TestReviewSpecGenerator:
    ...
    async def test_fetch_pr_comments_tool_success(
        self, mock_http_client, sample_pr_comments: list[dict[str, Any]]
    ) -> None:
    ...
    async def test_fetch_pr_comments_with_pagination(
        ...
        page1_response = create_mock_response(
            ...
            headers={
                "Link": (
                    '<https://api.github.com/repos/owner/repo/pulls/123/comments?page=2>; '  # noqa: E501
                    'rel="next"'
                )
            },
        )
        ...
        comments = await fetch_pr_comments(
            "owner", "repo", 123, per_page=2, max_pages=5
        )
    ...
    def test_create_review_spec_file_generation(
        self, temp_review_specs_dir: Path, sample_pr_comments: list[dict[str, Any]]
    ) -> None:
...
class TestFetchPrComments:
    ...
    async def test_fetch_pagination_safety_limits(
        ...
        comment_list = [
            ...
            for i in range(50)  # Smaller than limit
        ]
    ...
    @pytest.mark.asyncio
    async def test_fetch_no_github_token(
        self, no_github_token, mock_http_client
    ) -> None:
...
class TestCoverageBoost:
    ...
    @pytest.mark.asyncio
    async def test_fetch_comments_with_link_header(
        self, mock_http_client, github_token: str
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_fetch_comments_request_error_retry(
        self, mock_http_client, github_token: str
    ) -> None:
        ...
        create_mock_response(
            raise_for_status_side_effect=httpx.RequestError(
    ...
    @pytest.mark.asyncio
    async def test_fetch_comments_auth_fallback(
        self, mock_http_client, github_token: str
    ) -> None:
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_network_timeout_handling(self, mock_http_client) -> None:
